...
class CameramanModel(object):
    ...
    def get_kalman_predict(self, name, observe):
        ...
        if name not in self.kalman_filters:
            ...
            self.kalman_filters[name] = KalmanFilter1D(
                initial_x=observe,
    ...
    def parse_noise_player(self, positions, threshold=200):
        ...
        if (
            last_min - positions[0] > threshold
            and positions[1] - positions[0] > threshold
        ):
            positions.remove(min(positions))
        if (
            positions[-1] - last_max > threshold
    ...
    def predict(self, player_positions, ball_positions, debug_mode=False):
        ...
        player_mean_speed = self.get_kalman_predict(
        ...
        side_speed = (
            player_max_speed
            if abs(player_max_speed) > abs(player_min_speed)
            else player_min_speed
        )
        speed = bound(
            np.mean([side_speed, player_mean_speed]), -self.speed_max, self.speed_max
        )

        self.focus_slider = (
            bound(speed, -self.speed_max, self.speed_max) / self.speed_max
        ) / 2 + 0.5
        ...
        player_focus = mean_player_pos + (
            (abs(self.focus_slider - 0.5) * 3 + 0.5) / 2.0
        ) * (focus_side - mean_player_pos)

        ball_detected = len(ball_positions)

        ball_pos = (
            np.mean([self.ball_pos_memory[-1], player_focus])
            if len(ball_positions) == 0
            else np.mean(ball_positions)
        )
        ...
        target_x = bound(
            target_x,
            self.left_most - self.buffer_pixels,
            self.right_most + self.buffer_pixels,
        )
...
def court_area(points):
    ...
    print(
        abs(points_left[0][0] - points_right[0][0]),
        abs(points_left[1][0] - points_right[1][0]),
        abs(points_left[0][1] - points_left[1][1]),
    )
    area = (
        (
            abs(points_left[0][0] - points_right[0][0])
