    ...
    def draw_texts(self, img, texts):
        for i, text in enumerate(texts):
            cv2.putText(
                img,
                text,
                (self.position[0], self.position[1] + i * self.line_spacing),
    ...
    def get():
        ...
        if local_img_queue.qsize() < init_frames:
            for i in range(init_frames):
                local_img_queue.put(
                    img
                )  # 为了减少cameraman中滤波引起的延迟，展示的frame queue中预先多放几十帧，这样形成错位就实现了相当于用未来的检测结果来跟随当前帧
        local_img_queue.put(img)
...
def detect(
    ...
    def task():
        ...
        player_points = player_detector.predict(
            img,
            draw=False,
            crop_box=points_config[f"{direction}_crop_size"],
            direction=direction,
        )
        player_points = player_detector.map_points(
            player_points,
            stitcher,
            direction=direction,
            court_points=points_config["polygon"],
        )

        ball_points = ball_detector.predict(
            img,
            draw=False,
            crop_box=points_config[f"{direction}_crop_size"],
            direction=direction,
        )
        ball_points = ball_detector.map_points(
            ball_points,
            stitcher,
            direction=direction,
            court_points=points_config["polygon"],
...
def cameraman(
    ...
    queue_camera_man_result,
    queue_camera_man_result_left,
    queue_camera_man_result_right,
    stop_flag,
    debug=False,
):
...
def transform_frame(
    ...
    def task():
        ...
        if debug:
            ...
            texts = [
                f"target_x : {target_x}",
                f"target_x_filtered : {target_x_filtered}",
                f"mean_player_pos: {mean_player_pos}",
                f"player_speed : {player_speed}",
                f"player_focus : {player_focus}",
                f"ball_focus : {ball_focus}",
                f"max_player_pos : {max_player_pos}",
                f"min_player_pos : {min_player_pos}",
                f"ball_detected: {ball_detected}",
                f"focus_slider: {focus_slider}",
                f"player_positions: {player_positions}",
            ]
