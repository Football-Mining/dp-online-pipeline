...
class ImageTransformer:
    ...
    def init_remap(self):
        ...
        try:
            init_panorama = init_panorama.get()
        except:
            print("init_panorama.get() error")
            pass
...
def gpu_remap(image, remap_matrix_x, remap_matrix_y, interpolation=cv2.INTER_NEAREST):
    ...
    # print("upload image time", time.time() - t0)
...
#     points_config = json.load(open(f"camera_configs/{device_id}/{match_id}/points_config.json", "r"))
...
#     t0 = time.time()
#     left = img_transformer.compute_img(left_img, x, "left")
#     t1 = time.time()
#     right = img_transformer.compute_img(right_img, x, "right")
#     t2 = time.time()
#     test_result = img_transformer.transform(left, right, x)
#     t3 = time.time()
#     print(f"{t1-t0:.3f}s, {t2-t1:.3f}s, {t3-t2:.3f}s")
