...
def lonlat2XY(lonlat, shape):
    ...
    lst = [X, Y]
    out = np.concatenate(lst, axis=-1, dtype=np.float32)
...
class EquirectangularModel:
    ...
    def set_funcs_with_init_settings(
        ...
        y_func_left = lambda x: (
            left_most_setting[1]
            + (middle_point_setting[1] - left_most_setting[1])
            * (x - left_most_setting[0])
            / (middle_point_setting[0] - left_most_setting[0])
        )
        y_func_right = lambda x: (
            middle_point_setting[1]
            + (right_most_setting[1] - middle_point_setting[1])
            * (
                1
    ...
    def get_mat(self, x, y=None, fov=None):
        ...
        if fov is None:
            fov = self.fov_func(x)
        print("get fov time: {}".format(time.time() - t1))
        t2 = time.time()
        self.get_xyz(fov)
        print("get xyz time: {}".format(time.time() - t2))
        t3 = time.time()
        self.get_matrix(THETA, PHI)
        print("get matrix time: {}".format(time.time() - t3))
        return self.XY
...
if __name__ == "__main__":
    ...
    # # """创建实例"""
    e = EquirectangularModel((1274, 2930))
